...
Usage: $(basename $0) [OPTIONS]
 ...
 Options:
  -k            Inspect managed authentication keys only (APK or XAPK)
  -j            JSON report (requires -k)
...
if [ $# -eq 0 ]; then
	...
	while getopts "hf:vekj" option; do
	  case ${option} in
		h ) usage
			exit 0
...
source "$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)/managed_keys.sh"
if [ "$KEYS_JSON" = true ] && [ "$KEYS_ONLY" != true ]; then
    echo "The -j option requires -k." >&2
    exit 2
fi
if [ "$KEYS_ONLY" = true ]; then
    run_managed_keys "$APK"
    exit $?
fi
run_managed_keys "$APK" || echo "Managed key coverage is incomplete; continuing legacy checks." >&2
